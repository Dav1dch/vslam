...
int main(int argc, char **argv) {
    ...
    // Vector3d in fact is Eigen::Matrix<double, 3, 1>
    Vector3d v_3d;
    Matrix<float, 3, 1> vd_3d;
    // Matrix3d in fact is E igen::Matrix<double, 3, 3>
    Matrix3d matrix_33 = Matrix3d::Zero();

    matrix_23 << 1, 2, 3, 4, 5, 6;
    cout << "matrix 2x3 from 1 to 6: \n" << matrix_23 << endl;
    // use () to index the element
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 3; j++) {
            cout << matrix_23(i, j) << "\t";
        }
    ...
    // We cannot mix to different types of matrix
    // We need to im explicit transfomr it
